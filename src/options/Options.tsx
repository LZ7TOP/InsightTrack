...
export default function Options() {
  ...
  const [sortBy, setSortBy] = useState<'active' | 'open' | 'visits' | 'focus'>(
    'active',
  )
  ...
  async function updateSettingField<K extends keyof AppSettings>(
    key: K,
    value: AppSettings[K],
  ) {
  ...
  const fullSiteList: SiteListItem[] = Object.entries(domainMap).map(
    ([domain, stats]) => {
      const focusRate =
        stats.open > 0
  ...
  const siteActiveTimes = domainStatsList
    .slice(0, 12)
    .map((d) => d.activeTimeMs)
  ...
  const aggregatedPageList = Object.values(pageMap).sort(
  ...
  async function handleRemoveBlacklist(item: string) {
    const newB = settings.blacklist.filter(
      (b) => cleanDomain(b) !== cleanDomain(item),
    )
    await updateSettingField('blacklist', newB)
  }
  ...
  function handleExportData() {
    ...
    downloadAnchor.setAttribute(
      'download',
      `insighttrack_backup_${getLocalDateStr()}.json`,
    )
        ...
        {activeTab === 'overview' && (
          <div className='space-y-6'>
            ...
            <div className='bg-white border border-slate-200 p-6 rounded-2xl shadow-sm'>
              ...
              <ReactECharts
                option={websiteComparisonChartOption}
                style={{ height: '320px' }}
              />
            </div>
                  ...
                  <table className='w-full text-left text-xs'>
                    <thead className='bg-slate-50 text-[#64748B] font-bold'>
                      <tr>
                        ...
                        <th className='p-3.5 rounded-r-xl text-center'>
                          快捷操作
                        </th>
                      </tr>
                    </thead>
                ...
                <table className='w-full text-left text-xs'>
                  ...
                  <tbody className='divide-y divide-slate-100'>
                    {aggregatedPageList.slice(0, 20).map((page, idx) => {
                      ...
                      const sortedSessions = [...page.sessions].sort(
                      ...
                      return (
                        <Fragment key={idx}>
                          ...
                          >
                            ...
                            <td className='p-3 text-[#2563EB] font-bold'>
                              {formatMs(page.activeMs)}
                            </td>
                            <td className='p-3 text-[#64748B] font-medium'>
                          ...
                          {isExpanded && (
                            <tr className='bg-slate-50/80'>
                              <td
                                colSpan={7}
                                className='p-4 border-t border-b border-blue-100'
                              >
              ...
              <div className='space-y-4'>
                <div className='flex items-center justify-between'>
                  ...
                  <CustomSelect
                    ...
                    onChange={(val) =>
                      updateSettingField('idleThresholdSeconds', Number(val))
                    }
                    className='w-56'
                  />
                </div>
                ...
                <div className='flex items-center justify-between border-t border-slate-100 pt-3'>
                  <div>
                    ...
                    <span className='text-[11px] text-[#64748B] block mt-0.5'>
                      在工具栏扩展图标角标上实时展示当前网页今日活跃使用时长（如:
                      15s / 42m）
                    </span>
                  </div>
                  <button
                    onClick={() =>
                      updateSettingField('showBadge', !settings.showBadge)
                    }
                ...
                <div className='flex items-center justify-between border-t border-slate-100 pt-3'>
                  <div>
                    ...
                    <span className='text-[11px] text-[#64748B] block mt-0.5'>
                      如将 blog.github.com 自动无缝归并到 github.com
                      主域名下统一统计
                    </span>
                  </div>
                  <button
              ...
              <div className='space-y-4'>
                <div className='flex items-center justify-between'>
                  ...
                  <CustomSelect
                    ...
                    onChange={(val) =>
                      updateSettingField('dailyGoalHours', Number(val))
                    }
                    className='w-44'
                  />
                </div>
